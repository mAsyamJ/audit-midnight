...
- If you find an invariant break that requires multi-transaction ordering, message the **Adversarial Sequences** auditor
...
- If you find arithmetic assumptions that can be broken via multi-tx ordering, message the **Adversarial Sequences** auditor
...
- If you find a reentrancy/trust issue that depends on multi-step orchestration, message the **Adversarial Sequences** auditor
...
Only message teammates about CRITICAL or HIGH findings or cross-domain state dependencies. Write MEDIUM/LOW/INFO observations to your output file only.
- If a sequence relies on accounting drift, message the **State Consistency** auditor
- If a sequence relies on precision/rounding behavior, message the **Math & Rounding** auditor
- If a sequence relies on callbacks/reentrancy/trust assumptions, message the **Reentrancy & Trust** auditor
- If another auditor messages you about a finding, test whether it can be escalated into a multi-step exploit sequence and respond
- Write all cross-cutting findings to your output file regardless of whether you also message teammates
- Use the format: "CROSS-CUTTING: [finding summary]. Impacts your analysis because [reason]. See {output_file} finding #{N}."
