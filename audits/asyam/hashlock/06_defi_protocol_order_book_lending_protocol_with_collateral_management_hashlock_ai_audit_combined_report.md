# DeFi Protocol - Order Book / Lending Protocol with Collateral Management — Hashlock AI Audit Combined Report

Source title: **DeFi Protocol - Order Book / Lending Protocol with Collateral Management**  
Source date shown in scan: **May 30, 2026**  
Scope shown in scan: **8 files, 20 issues found**  
Access-control model shown in scan: **role-based**  
Purpose: convert the latest Hashlock AI scan into a single cumulative Markdown audit file for Morpho Midnight competition research.

> Important: this is based on an AI-generated scan and can contain false positives. Treat every issue as a research hypothesis until it is proven with source-code line references, a Foundry PoC, and deduplication against existing comments, tests, Certora specs, and audit reports.

---

## Executive Summary

This scan covers a broader Midnight surface than the earlier interface/periphery scans. It describes Midnight as an order-book / fixed-rate lending protocol with collateral management, settlement fees, continuous fees, liquidations, offer/order ratification, EIP-712 authorization, Permit2, ERC20 permit support, and event/constants libraries.

The scan’s highest-severity claims concentrate around:

```text
1. ECDSA / ecrecover signature verification edge cases.
2. Bundle/periphery reentrancy and authorization validation.
3. Ratifier cancellation / unratification access control.
4. Unbounded arrays in bundle operations.
5. Permit griefing.
6. ConstantsLib boundary/gas issues.
7. Event indexing and monitoring limitations.
```

Deep triage: many items are **interface-level warnings**, not confirmed implementation bugs. The strongest candidates are the ones that can be tested against real implementation behavior:

```text
- bundle reentrancy / temporary token-balance corruption,
- taker/onBehalf authorization checks in actual MidnightBundles,
- ERC2612 permit front-running tolerance,
- root cancellation and unratification authority,
- ConstantsLib maxSettlementFee bounds and maxLif denominators,
- event indexing if monitoring requirements are in scope.
```
...
The scan states **8 files** and covers components including:
...
It also references the Midnight core protocol, Permit2, ERC20 tokens, ratifiers, EIP-712 signatures, off-chain Merkle trees, and oracle feeds.

---

## Access Control and External Dependency Map

### Privileged roles from scan
...
3. tickSpacingSetter
4. feeClaimer
5. MIDNIGHT core contract
6. maker / offer creator
7. authorized delegated operator
```
...
MIDNIGHT
Permit2 / Uniswap
ERC20 loan and collateral tokens
IRatifier implementations
ISetterRatifier
IEcrecoverRatifier
...
Permit2
Oracle / price feed
Off-chain order-book infrastructure
```
...
| Scan claim | Initial competition value | Triage |
|---|---|---|
| Unexpected `ecrecover` null address in authorizer | Medium if implementation missing zero check | Need inspect actual implementation, not interface only. |
| Unexpected `ecrecover` null address in ratifier | Likely false positive if actual code checks signer != 0 | Earlier source showed `EcrecoverRatifier` checks `_signer != address(0)`. |
| Bundle reentrancy via ERC777 / callback tokens | High-priority test | Must prove real token balance corruption or state break. |
| Missing `taker == msg.sender || authorized` validation | Likely false positive if implementation checks it | Earlier periphery source showed this check exists in bundle functions. |
| Minimal EIP-712 domain separator | Low/Medium design | Need prove replay across real deployment/fork assumptions. |
| Signature malleability | Low unless signature bytes are used as replay key | Root/nonce replay protection likely dominates. |
| Unbounded bundle arrays | Low/Medium self-DoS | Strong only if third party controls arrays or causes persistent effects. |
| ERC20 permit front-running | Medium only if implementation does not tolerate consumed permit | Earlier periphery code catches ERC2612 permit failure and then transfers. |
| Unauthorized `cancelRoot` | Likely false positive if implementation checks maker or authorized | Earlier source showed maker or Midnight authorization check. |
| `setIsRootRatified` unratify DoS | Medium design risk | Real because delegated operator can toggle; depends authorization model. |
| Floating pragmas | Low/Info | Not a main competition target. |
| Zero referral recipient | Low | User-error / UX. |
| `maxSettlementFee` index panic | Low unless caller lacks prior validation | Core fee setters likely validate index <= 6 before call. |
| `maxLif` division by zero | Likely low/false positive if LLTV/cursor constants bounded | Needs symbolic/fuzz validation. |
| Event indexing | Informational | Useful monitoring issue only. |
| `LIQUIDATION_LOCK_SLOT` collision | Informational / likely false positive | Keccak slot collision is negligible in non-upgradeable contract. |

---

# Detailed Findings and Deep Triage

## High-01 — Unexpected `ecrecover` Null Address in `EcrecoverAuthorizer` Signature Verification

**File:** `IEcrecoverAuthorizer.sol`  
**Scan severity:** High  
**Research status:** Needs implementation inspection.

### Claim

If the actual authorizer implementation recovers a signer using raw `ecrecover` and compares the recovered signer to `authorization.authorizer` without checking `recoveredSigner != address(0)`, an invalid signature could pass when `authorization.authorizer == address(0)`.

### Deep triage

The uploaded scan only shows the **interface** function:

```solidity
function setIsAuthorized(Authorization memory authorization, Signature memory signature) external;
```

The interface cannot prove the bug. This must be validated against the actual implementation contract if present in scope.

### Validation checklist

```text
1. Locate EcrecoverAuthorizer implementation.
2. Check ecrecover usage.
3. Confirm whether recovered signer is checked against address(0).
4. Confirm nonce and deadline are enforced before authorization mutation.
5. Confirm address(0) cannot create meaningful authorization in Midnight.
```

### PoC plan

```text
- Build Authorization(authorizer=address(0), authorized=attacker, isAuthorized=true).
- Use malformed signature with v=0.
- Call setIsAuthorized.
- Assert whether Midnight.isAuthorized(address(0), attacker) becomes true.
```

### Likely final severity

```text
High/Medium if actual implementation accepts zero-recovered signatures and this creates meaningful delegated power.
Invalid if implementation uses ECDSA.recover or explicit zero check.
```

---

## High-02 — Unexpected `ecrecover` Null Address in `EcrecoverRatifier`

**File:** `IEcrecoverRatifier.sol` / `EcrecoverRatifier.sol`  
**Scan severity:** High  
**Research status:** Likely false positive against actual implementation.

### Claim

A malformed signature could recover `address(0)` and pass if maker is `address(0)`.

### Deep triage

Earlier source inspection of `EcrecoverRatifier.sol` showed the actual implementation performs:
...
Therefore, the null-address recovery bypass is likely **not valid** for the actual ratifier.

### Remaining angle

A smaller defense-in-depth issue may remain if `offer.maker == address(0)` and `IMidnight.isAuthorized(address(0), signer)` can be true, but that would require a valid signer authorized by zero address and meaningful zero-address offers.

### Likely final severity

```text
Invalid / Low defense-in-depth.
```

---

## High-03 — Potential Reentrancy in Bundle Operations via ERC777 / Callback Tokens

**File:** `IMidnightBundles.sol` / `MidnightBundles.sol`  
**Scan severity:** High  
**Research status:** High-priority PoC target.

### Claim

Bundle functions perform multiple external calls through permits, tokens, Midnight core calls, and callbacks. Malicious ERC777/callback tokens may reenter the periphery and corrupt execution assumptions.

### Deep triage

This overlaps with the previous periphery scan and is one of the better research targets. However, the protocol’s token safety requirements may exclude reentrant tokens. The winning path is not just “ERC777 can reenter”; it is:

```text
A token or callback satisfying accepted assumptions can cause temporary balance corruption, incorrect min/max validation, or unauthorized transfer.
```

### PoC plan

```text
test/poc/PoC_BundlerReentrancy.t.sol

1. Create malicious collateral token with transferFrom hook.
2. Create malicious callback or ratifier path if easier.
3. Reenter bundle during:
   - pullToken
   - forceApproveMax
   - Midnight.take
   - Midnight.supplyCollateral
4. Assert attacker balance increases or victim receives less.
5. If transaction reverts and rolls back, classify as DoS only.
```

---

## High-04 — Missing Validation That `taker` Matches `msg.sender` in Bundle Functions
...
If implementation does not validate `taker == msg.sender || isAuthorized(taker, msg.sender)`, attacker can execute bundles on behalf of victims.

### Deep triage

Earlier source inspection of `MidnightBundles.sol` showed bundle functions contain:

```solidity
require(taker == msg.sender || IMidnight(MIDNIGHT).isAuthorized(taker, msg.sender), Unauthorized());
```

and `repayAndWithdrawCollateral` similarly checks `onBehalf == msg.sender || isAuthorized(onBehalf, msg.sender)`.

Therefore, the broad claim is likely **invalid** against actual code.
...
```text
Is Midnight authorization too broad for bundler operations?
Can an authorized relayer misuse Permit2 or receiver parameters?
```

This should be reframed as an **authorization-scope / Permit2-context** issue, not missing validation.

---

## Medium-01 — Missing Cross-Chain Replay Protection in `EcrecoverAuthorizer` Domain Separator
...
The domain separator uses only:
...
`chainId + verifyingContract` is usually sufficient to bind a signature to a chain and contract address. Missing name/version is less ergonomic and less future-proof, but not automatically replayable.

The strongest validation is to prove:

```text
Same verifying contract address + same chainId + changed semantics causes old signatures to remain valid in a materially harmful way.
```

If the contract is immutable/non-upgradeable, this is weak.

---

## Medium-02 — Signature Malleability in `EcrecoverAuthorizer` and `EcrecoverRatifier`

**Files:** `IEcrecoverAuthorizer.sol`, `IEcrecoverRatifier.sol`  
**Scan severity:** Medium

### Claim

Raw `ecrecover` accepts high-s malleable signatures unless checked.

### Deep triage

This is technically true for raw `ecrecover`. Severity depends on replay model:

```text
- Authorizer should use nonce, so malleable signature should not replay after nonce used.
- Ratifier uses root cancellation and offer caps, not signature uniqueness.
- If any off-chain/indexer logic deduplicates by raw signature bytes, malleability can confuse monitoring but not necessarily steal funds.
```

### Suggested validation

```text
- Construct low-s and high-s signature variants for same digest.
- Confirm both pass EcrecoverRatifier if no low-s check.
- Determine whether second signature changes any on-chain state or bypasses replay/cancellation.
```

Likely **Low/Medium** unless signature bytes are used as replay keys.

---

## Medium-03 — DoS via Unbounded Array Iteration in Bundle Functions
...
Bundle functions accept unbounded `takes`, `collateralWithdrawals`, and `collateralSupplies`.

### Deep triage

This is mostly self-DoS because the caller supplies arrays and pays gas. It becomes stronger if:

```text
- an external protocol builds arrays from attacker-controlled input,
- partial state persists across failure,
- the bundler holds funds before a large loop can revert,
- a victim signs/authorizes a route that a relayer can bloat.
```

Likely Low/Medium design issue.

---

## Medium-04 — ERC20 Permit Griefing Attack via Front-Running

**File:** `IERC20Permit.sol` / `MidnightBundles.pullToken`  
**Scan severity:** Medium  
**Research status:** Likely already mitigated in implementation.

### Claim

A front-runner can consume an ERC2612 permit before the bundle transaction, causing bundle revert.

### Deep triage

Earlier `MidnightBundles.pullToken` source showed the ERC2612 permit call is wrapped in `try/catch` and then `safeTransferFrom` is attempted. This is exactly the standard mitigation: if permit was already used but allowance is now set, the transfer should still work.

Therefore this is likely **false positive** for ERC2612. Still test:

```text
1. User signs permit.
2. Attacker submits permit first.
3. User bundle executes.
4. If allowance is sufficient, bundle should succeed.
```

---

## Medium-05 — Unauthorized `cancelRoot`: Any Address Can Cancel Another Maker’s Root

**File:** `IEcrecoverRatifier.sol` / `EcrecoverRatifier.sol`  
**Scan severity:** Medium  
**Research status:** Likely false positive as written.

### Claim

If access control is insufficient, anyone or a privileged address could cancel roots for makers.

### Deep triage

Earlier actual implementation showed:

```solidity
require(maker == msg.sender || IMidnight(MIDNIGHT).isAuthorized(maker, msg.sender), Unauthorized());
```

So not “any address”. The real design issue is that **authorized delegates** can cancel roots. That is documented by the authorization model and may be intended.

### Stronger framing

```text
Authorized delegate can permanently cancel maker roots in EcrecoverRatifier.
```

This is a griefing/design-risk issue, not missing access control.

---

## Medium-06 — `setIsRootRatified` Can Be Called to Unratify Active Roots

**File:** `ISetterRatifier.sol` / `SetterRatifier.sol`  
**Scan severity:** Medium  
**Research status:** Plausible design risk.

### Claim

`setIsRootRatified(maker, root, false)` can invalidate active roots. If an authorized delegate is compromised/malicious, it can DoS a maker’s active orders.

### Deep triage

This overlaps with previous scan. The actual function supports both true and false, and authorized delegates may call it. This is likely real but severity depends on whether users are clearly warned that delegates have full root-management authority.

### PoC plan

```text
- Maker authorizes delegate.
- Maker ratifies root.
- Delegate calls setIsRootRatified(..., false).
- Attempt to fill offer under root.
- Assert NotRatified.
```

Likely **Low/Medium griefing**, not High, unless a trusted official delegate can be abused.

---

## Low-01 — Floating Pragma Allows Compilation With Vulnerable Compiler Versions

**Files:** `IERC20Permit.sol`, `IPermit2.sol`, `IMidnightBundles.sol`, `IEcrecoverAuthorizer.sol`  
**Scan severity:** Low

Valid code-quality issue, but low-value for competition.

---

## Low-02 — Missing Cross-Chain Replay Protection in `EcrecoverRatifier` Domain Separator

**File:** `IEcrecoverRatifier.sol`  
**Scan severity:** Low

This duplicates Medium-01 style domain concern. Since actual domain includes `chainId` and verifying contract, this is likely Low/Info unless redeploy-at-same-address semantics are realistic.

---

## Low-03 — Referral Fee Recipient Can Be Zero Address

**File:** `IMidnightBundles.sol` / `MidnightBundles.sol`  
**Scan severity:** Low

Valid UX/fund-loss-by-user-error issue. Stronger only if a relayer controls `referralFeeRecipient` while user expects no burn.

---

## Low-04 — `maxSettlementFee` Array Index Out of Bounds Risk
...
`maxSettlementFee(index)` uses a seven-element inline array and panics if index >= 7.

### Deep triage

Core fee setter functions likely check `index <= 6` before calling `maxSettlementFee`. If all internal calls validate index first, this is not a security bug.

### Validation

```text
- Search every call to maxSettlementFee.
- Confirm every user-controlled index is checked <= 6 first.
- If any unchecked path exists, produce revert/DoS PoC.
```

---

## Low-05 — `maxLif` Division by Zero Risk
...
`maxLif(lltv, cursor)` could divide by zero if denominator becomes zero.

### Deep triage

The scan’s own examples mostly show safe denominators. Need fuzzing over actual allowed LLTVs and actual liquidation cursor constants. If the function is only called with bounded constants, this is likely false positive.

### Validation

```text
- Fuzz allowed lltv values and LIQUIDATION_CURSOR_LOW/HIGH.
- Assert denominator > 0.
- Search all maxLif callers.
```

---

## Gas-01 — Inline Array in `maxSettlementFee` Allocates Memory

**File:** `ConstantsLib.sol`  
**Scan severity:** Gas

Valid gas optimization. Replace with if-chain if gas matters. Not competition priority.

---

## Informational-01 — `MAX_CONTINUOUS_FEE` Precision Loss Due to Integer Division

**File:** `ConstantsLib.sol`  
**Scan severity:** Informational

Valid observation. Truncation is conservative and extremely small, so not security-relevant.

---

## Informational-02 — Inconsistent Pragma Versions Between Core and Periphery

**File:** multiple  
**Scan severity:** Informational

Same family as Low-01. Code-quality only.

---

## Informational-03 — `Take` Event Missing Indexed Fields for Critical Parameters
...
`Take` indexes `id_`, `taker`, and `maker`, but not `caller` or `group`.

### Deep triage

This is a legitimate monitoring/analytics issue. It can matter for incident response, but not usually a financial vulnerability.

Potential valid recommendation:

```text
Emit a secondary event optimized for monitoring group/caller.
```

---

## Informational-04 — `LIQUIDATION_LOCK_SLOT` Uses Keccak String Slot

**File:** `ConstantsLib.sol`  
**Scan severity:** Informational

Likely safe. Keccak-derived slots are standard. Only relevant if upgradeable/proxy storage layout exists. Midnight core appears immutable/non-proxy from prior context, so likely not a real issue.
...
DeFi Protocol - Order Book / Lending Protocol with Collateral Management

This codebase represents a DeFi protocol called 'Midnight' (by Morpho Association) that appears to be a sophisticated order-book or lending/borrowing protocol with collateral management, settlement fees, continuous fees, liquidation mechanics, and a ratification system for offers/orders. The contracts include interfaces for ratifiers (signature-based and setter-based), periphery bundles for complex multi-step operations, ERC20 permit support, Permit2 integration, and a comprehensive event/constants library. The system uses a maker/taker model with 'roots' (likely Merkle roots of offer trees), authorization via EIP-712 signatures, and a tick-spacing mechanism reminiscent of concentrated liquidity AMMs.
...
feeClaimer
5
MIDNIGHT (core contract)
6
maker (offer creator)
7
authorized (delegated operator)
...
Permit2 (Uniswap)
3
ERC20 tokens (loan tokens, collateral tokens)
4
IRatifier implementations (ISetterRatifier, IEcrecoverRatifier)
5
IEcrecoverAuthorizer
...
IEcrecoverAuthorizer.sol
Unexpected ecrecover Null Address in EcrecoverAuthorizer Signature Verification
The `IEcrecoverAuthorizer.setIsAuthorized` function uses `ecrecover` to recover the signer from an EIP-712 signature. The `ecrecover` precompile returns `address(0)` when given an invalid signature (e.g., when `v` is not 27 or 28, or when the signature parameters are malformed). If the implementation does not explicitly check that the recovered address is non-zero before comparing it to `authorization.authorizer`, an attacker could craft a signature that causes `ecrecover` to return `address(0)`. If any `authorizer` address in the system is `address(0)` (e.g., uninitialized storage), this could allow unauthorized authorization delegation. Furthermore, if the check is `recoveredAddress == authorization.authorizer` without a zero-address guard, passing `authorizer = address(0)` with an invalid signature would pass the check.

Impact
An attacker could potentially bypass signature verification by crafting an invalid signature that causes `ecrecover` to return `address(0)`, then setting `authorization.authorizer = address(0)`. This could allow unauthorized delegation of permissions in the MIDNIGHT core contract, enabling an attacker to act on behalf of the zero address.
Scenario
1. Attacker sets `authorization.authorizer = address(0)`
2. Attacker crafts an invalid signature with `v = 0` (not 27 or 28)
3. `ecrecover` returns `address(0)` for the invalid signature
4. If the implementation checks `recoveredSigner == authorization.authorizer` without a zero-address check, this passes
5. Attacker successfully sets authorization for `address(0)` as the authorizer

Note: The practical impact depends on whether `address(0)` has any meaningful role in the system.
Affected code
function setIsAuthorized(Authorization memory authorization, Signature memory signature) external;
// Authorization struct contains: address authorizer, address authorized, bool isAuthorized, uint256 nonce, uint256 deadline
Proposed fix
Always validate that the recovered address from `ecrecover` is non-zero before using it for authorization:
function setIsAuthorized(Authorization memory authorization, Signature memory signature) external {
    ...
    address recoveredSigner = ecrecover(digest, signature.v, signature.r, signature.s);

    // CRITICAL: Check for zero address
    if (recoveredSigner == address(0)) revert InvalidSignature();
    if (recoveredSigner != authorization.authorizer) revert InvalidSignature();

    // ... rest of logic
}
Alternatively, use OpenZeppelin's `ECDSA.recover` which includes this check.
2

IEcrecoverRatifier.sol
Unexpected ecrecover Null Address in EcrecoverRatifier
The `IEcrecoverRatifier` uses `ecrecover` to verify that a maker has signed an offer root. Similar to the EcrecoverAuthorizer, if the implementation does not check that the recovered address is non-zero, an attacker could submit a malformed signature (with `v` set to an invalid value like 0 or 1) causing `ecrecover` to return `address(0)`. If the maker address being checked is `address(0)` or if there's no zero-address guard, this could allow unauthorized ratification of arbitrary offer roots.

Impact
An attacker could potentially ratify arbitrary offer roots for `address(0)` as the maker, or if the zero-address check is missing, bypass signature verification entirely. This could allow execution of unauthorized orders in the MIDNIGHT protocol.
Scenario
1. Attacker constructs `ratifierData` with `maker = address(0)` and an invalid signature (`v = 0`)
2. `ecrecover` returns `address(0)` for the invalid signature
3. If implementation checks `recoveredSigner == maker` without zero-address guard, this passes
4. Attacker successfully ratifies an offer root for `address(0)`
5. If `address(0)` has any offers in the system, they could be executed
Affected code
// IEcrecoverRatifier - ratify function (inherited from IRatifier)
// Uses ecrecover to verify maker's signature over a root
// EIP712_DOMAIN_TYPEHASH = 0x47e79534a245952e8b16893a336b85a3d9ea9fa8c573f3d803afb92a79469218
Proposed fix
Ensure the `ratify` implementation includes a zero-address check on the `ecrecover` result:
address recoveredMaker = ecrecover(digest, sig.v, sig.r, sig.s);
if (recoveredMaker == address(0)) revert InvalidSignature();
if (recoveredMaker != maker) revert Unauthorized();
Consider using OpenZeppelin's `ECDSA` library which handles this automatically.
3

IMidnightBundles.sol
Potential Reentrancy in Bundle Operations via ERC777/Callback Tokens
The `IMidnightBundles` bundle functions perform multiple sequential external calls: token permits, `MIDNIGHT.take`, `MIDNIGHT.supplyCollateral`, `MIDNIGHT.withdrawCollateral`, and `MIDNIGHT.repay`. If any of the tokens involved are ERC777 tokens (which have transfer hooks) or tokens with callbacks, a malicious token could reenter the bundle contract during execution. For example, during `supplyCollateral`, if the collateral token is ERC777, the `tokensToSend` hook could reenter `buyWithUnitsTargetAndWithdrawCollateral` before the first call completes. The `LIQUIDATION_LOCK_SLOT` constant in `ConstantsLib.sol` suggests the core MIDNIGHT contract has some reentrancy protection, but the periphery bundle contract may not.

Impact
A malicious ERC777 collateral token could reenter the bundle contract during a collateral supply operation, potentially allowing double-spending of collateral, manipulation of the units target tracking, or bypassing of the `minSellerAssets` check. This could result in financial loss for the taker.
Scenario
1. Attacker creates a malicious ERC777 token with a `tokensToSend` hook
2. Attacker calls `supplyCollateralAndSellWithUnitsTarget` with the malicious token as collateral
3. During `MIDNIGHT.supplyCollateral`, the ERC777 hook fires
4. Hook reenters `supplyCollateralAndSellWithUnitsTarget` with different parameters
5. Inner call completes, modifying state (units taken, assets received)
6. Outer call continues with stale state, potentially violating `minSellerAssets` invariant
Affected code
...
CollateralSupply[] memory collateralSupplies, // each may trigger token transfer
Take[] memory takes, // each calls MIDNIGHT.take
uint256 referralFeePct,
address referralFeeRecipient
) external;
Proposed fix
Add a reentrancy guard to all bundle functions:
...
Also follow the checks-effects-interactions pattern: validate all inputs first, then perform state changes, then make external calls.
4

IMidnightBundles.sol
Missing Validation That taker Parameter Matches msg.sender in Bundle Functions
The `IMidnightBundles` bundle functions accept a `taker` address parameter that specifies who is taking the offers. If the implementation does not validate that `taker == msg.sender` (or that `msg.sender` is authorized to act on behalf of `taker`), any caller could execute bundle operations on behalf of any address. This could allow an attacker to force another user's address to take offers they didn't intend to, potentially draining their approved token balances or manipulating their positions. The `Unauthorized` error in the interface suggests some access control exists, but the exact validation is unclear.

Impact
If `taker` is not validated against `msg.sender`, an attacker could:
1. Force another user to take unfavorable offers, spending their approved tokens
2. Manipulate another user's collateral position
3. Execute collateral withdrawals on behalf of another user to a receiver they control
This could result in significant financial loss for the victim.
Scenario
1. Victim has approved the MidnightBundles contract to spend their loan tokens
2. Attacker calls `buyWithUnitsTargetAndWithdrawCollateral` with `taker = victim_address`
3. If no authorization check, the bundle executes using victim's tokens
4. Victim's tokens are spent on offers they didn't choose
5. Collateral is withdrawn to `collateralReceiver` (which attacker controls)
Affected code
function buyWithUnitsTargetAndWithdrawCollateral(
uint256 targetUnits,
uint256 maxBuyerAssets,
address taker, // who is taking the offers - could be any address
...
Proposed fix
Validate that the caller is authorized to act on behalf of the taker:
function buyWithUnitsTargetAndWithdrawCollateral(
    uint256 targetUnits,
    uint256 maxBuyerAssets,
    address taker,
    ...
    // Validate caller is authorized to act on behalf of taker
    if (msg.sender != taker && !IMidnight(MIDNIGHT).isAuthorized(taker, msg.sender)) {
        revert Unauthorized();
    }
...
IEcrecoverAuthorizer.sol
Missing Cross-Chain Replay Protection in EcrecoverAuthorizer Domain Separator
The `IEcrecoverAuthorizer` defines the same minimal `EIP712_DOMAIN_TYPEHASH` as the EcrecoverRatifier: `keccak256("EIP712Domain(uint256 chainId,address verifyingContract)")`. The same concerns apply: absence of `name` and `version` fields means signatures are less tightly bound to a specific contract version. Additionally, the `AUTHORIZATION_TYPEHASH` hardcoded value `0x81d0284fb0e2cde18d0553b06189d6f7613c96a01bb5b5e7828eade6a0dcac91` should be verified to match `keccak256("Authorization(address authorizer,address authorized,bool isAuthorized,uint256 nonce,uint256 deadline)")` exactly.

Impact
If the hardcoded `AUTHORIZATION_TYPEHASH` does not match the actual struct definition, all signature verifications would fail or produce incorrect results. Additionally, the minimal domain separator provides weaker replay protection compared to the full EIP-712 standard.
Scenario
Verify the hash: `keccak256("Authorization(address authorizer,address authorized,bool isAuthorized,uint256 nonce,uint256 deadline)")` should equal `0x81d0284fb0e2cde18d0553b06189d6f7613c96a01bb5b5e7828eade6a0dcac91`. Any mismatch would cause all authorization signatures to be invalid.
Affected code
...
Proposed fix
1. Verify the hardcoded hash values match their string representations using a test.
2. Consider computing these values dynamically in the constructor rather than hardcoding:
bytes32 public immutable AUTHORIZATION_TYPEHASH = keccak256(
    "Authorization(address authorizer,address authorized,bool isAuthorized,uint256 nonce,uint256 deadline)"
);
3. Include `name` and `version` in the domain separator for stronger binding.
2

IEcrecoverAuthorizer.sol
Signature Malleability in EcrecoverAuthorizer and EcrecoverRatifier
Both `IEcrecoverAuthorizer` and `IEcrecoverRatifier` use raw `ecrecover` for signature verification. Ethereum's ECDSA signatures are malleable: for any valid signature `(r, s, v)`, the signature `(r, n - s, v ^ 1)` (where `n` is the curve order) is also valid for the same message. This means an attacker can compute a second valid signature from an existing one without knowing the private key. While the `EcrecoverAuthorizer` uses nonces (which would prevent replay of the original signature), the malleable variant would have a different bytes representation and could potentially bypass signature-used checks if they're based on the raw signature bytes rather than the nonce. For the `EcrecoverRatifier`, which uses root-based cancellation rather than nonces, malleability could allow replaying a modified signature for the same root.

Impact
An attacker who observes a valid signature can compute a second valid signature for the same message. In the context of the EcrecoverRatifier, if signature uniqueness is used as part of replay protection (in addition to root cancellation), the malleable signature could bypass this check. For the EcrecoverAuthorizer, the nonce mechanism provides primary protection, but the malleable signature could be used to front-run or confuse off-chain monitoring systems.
Scenario
Given a valid signature `(v, r, s)` for a message:
1. Compute `s' = secp256k1_order - s`
2. Compute `v' = v ^ 1` (flip between 27 and 28)
3. The signature `(v', r, s')` is also valid for the same message
4. If the system tracks used signatures by their bytes representation, `(v', r, s')` would not be in the used-signatures set
5. Attacker submits the malleable signature to replay the authorization
Affected code
// IEcrecoverAuthorizer
function setIsAuthorized(Authorization memory authorization, Signature memory signature) external;

// IEcrecoverRatifier
// ratify function uses Signature struct { uint8 v; bytes32 r; bytes32 s; }
Proposed fix
Use OpenZeppelin's `ECDSA.recover` library which enforces that `s` is in the lower half of the curve order (preventing malleability) and that `v` is either 27 or 28:
...
// This will revert if s > secp256k1n/2 or v is invalid
Alternatively, manually check: `require(uint256(s) <= 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0, "Invalid s value");`
3

IMidnightBundles.sol
DoS via Unbounded Array Iteration in Bundle Functions
The `IMidnightBundles` interface exposes functions that accept unbounded arrays: `Take[] memory takes`, `CollateralWithdrawal[] memory collateralWithdrawals`, and `CollateralSupply[] memory collateralSupplies`. Each element in these arrays triggers external calls to the MIDNIGHT core contract (e.g., `take`, `supplyCollateral`, `withdrawCollateral`). If a caller passes an extremely large array, the transaction could exceed the block gas limit, causing it to always revert. While this primarily affects the caller themselves (self-DoS), it could also be used to grief specific users if the bundle contract is used in a way where others can influence the array size.

Impact
Transactions with very large arrays will fail due to gas exhaustion. While primarily a self-DoS risk, it could cause issues in automated systems or protocols that build on top of MidnightBundles. In extreme cases, if the bundle contract holds intermediate state, failed transactions could leave the contract in an inconsistent state.
Scenario
1. Attacker (or misconfigured bot) calls `buyWithUnitsTargetAndWithdrawCollateral` with `takes` array of 10,000 elements
2. Each iteration calls MIDNIGHT.take which involves storage reads/writes and potentially token transfers
3. Transaction exceeds block gas limit (~30M gas on Ethereum)
4. Transaction always reverts, wasting caller's gas
Affected code
function buyWithUnitsTargetAndWithdrawCollateral(
uint256 targetUnits,
uint256 maxBuyerAssets,
address taker,
TokenPermit memory loanTokenPermit,
Take[] memory takes, // unbounded
CollateralWithdrawal[] memory collateralWithdrawals, // unbounded
address collateralReceiver,
uint256 referralFeePct,
address referralFeeRecipient
) external;
Proposed fix
Add maximum array length checks at the beginning of each bundle function:
uint256 constant MAX_TAKES = 100;
uint256 constant MAX_COLLATERAL_OPERATIONS = 16; // matches MAX_COLLATERALS_PER_BORROWER
...
function buyWithUnitsTargetAndWithdrawCollateral(
    ...
    require(takes.length <= MAX_TAKES, "Too many takes");
    require(collateralWithdrawals.length <= MAX_COLLATERAL_OPERATIONS, "Too many withdrawals");
    // ... rest of logic
}
Note: `MAX_COLLATERALS_PER_BORROWER = 16` from `ConstantsLib.sol` provides a natural upper bound for collateral operations.
4

IERC20Permit.sol
ERC20 Permit Griefing Attack via Front-Running
The `IERC20Permit.permit` function is used in bundle operations to allow gasless token approvals. A well-known attack vector exists where a front-runner can observe a pending `permit` transaction in the mempool and call `permit` themselves before the original transaction, causing the original transaction to revert (since the nonce is now consumed). This is particularly relevant in `repayAndWithdrawCollateral` and buy/sell bundle functions where the permit is called as part of a larger atomic operation. If the permit call reverts, the entire bundle transaction reverts, causing DoS for the user.

Impact
A front-runner can grief users by front-running their permit transactions, causing bundle operations to revert. This is a DoS attack that forces users to use traditional approve+call patterns instead of the gasless permit flow. While not a fund loss vulnerability, it degrades user experience and could be used to prevent time-sensitive operations (e.g., repaying a loan before liquidation).
Scenario
1. User submits `repayAndWithdrawCollateral` with a `loanTokenPermit` containing an ERC2612 permit
2. Front-runner observes the transaction in the mempool
3. Front-runner extracts the permit signature and calls `token.permit(owner, spender, value, deadline, v, r, s)` directly
4. Front-runner's transaction is mined first, consuming the nonce
5. User's bundle transaction reverts because the permit nonce is already used
6. User cannot repay their loan in this transaction, potentially facing liquidation
Affected code
// IERC20Permit.sol
function permit(
address owner,
...
// Used in IMidnightBundles:
struct TokenPermit {
PermitKind kind; // ERC2612 or Permit2
bytes data;
}
Proposed fix
Implement permit calls in a try-catch pattern to handle the case where the permit has already been executed (nonce already consumed):
function _tryPermit(TokenPermit memory permit, address token, address owner, address spender, uint256 amount) internal {
    if (permit.kind == PermitKind.ERC2612) {
        ...
        try IERC20Permit(token).permit(owner, spender, value, deadline, v, r, s) {}
            ...
            // Permit may have already been executed; check if allowance is sufficient
            require(
                IERC20(token).allowance(owner, spender) >= amount,
                "Insufficient allowance after failed permit"
            );
        }
    }
}
This pattern is used by Uniswap's Universal Router and other protocols to handle front-running gracefully.
5

IEcrecoverRatifier.sol
Unauthorized cancelRoot - Any Address Can Cancel Another Maker's Root
The `IEcrecoverRatifier.cancelRoot` function takes `address maker` and `bytes32 root` as parameters. The interface defines an `Unauthorized` error, suggesting there is access control, but the function signature allows any caller to pass any `maker` address. If the implementation only checks `msg.sender == maker` (i.e., only the maker can cancel their own root), this is correct. However, if the implementation allows the MIDNIGHT contract or other privileged addresses to cancel roots on behalf of makers, this could be abused. The concern is whether the access control is correctly implemented to prevent unauthorized root cancellation.

Impact
If the access control on `cancelRoot` is insufficient (e.g., allows MIDNIGHT contract to cancel any maker's root), a compromised MIDNIGHT contract or privileged role could cancel all makers' roots, effectively shutting down all order execution in the protocol. This would be a severe DoS attack.
Scenario
1. Attacker gains control of the MIDNIGHT contract (or any address that passes the access check)
2. Attacker calls `cancelRoot(victimMaker, victimRoot)` for all active makers
3. All pending orders from those makers are invalidated
4. Protocol order book is effectively emptied
5. Makers must re-sign and re-submit all their orders
Affected code
/// @dev Allows a maker to cancel a root, preventing future ratification
function cancelRoot(address maker, bytes32 root) external;

/// ERRORS ///
error Unauthorized(); // suggests access control exists
Proposed fix
Ensure the `cancelRoot` implementation strictly checks `msg.sender == maker`:
function cancelRoot(address maker, bytes32 root) external {
    // Only the maker themselves can cancel their root
    if (msg.sender != maker) revert Unauthorized();

    isRootCanceled[maker][root] = true;
    emit CancelRoot(msg.sender, maker, root);
}
Do NOT allow the MIDNIGHT contract or any other address to cancel roots on behalf of makers, as this would create a centralization risk.
6

ISetterRatifier.sol
setIsRootRatified Can Be Called to Unratify Active Roots - Potential DoS
The `ISetterRatifier.setIsRootRatified` function accepts a `bool newIsRootRatified` parameter, meaning it can both ratify AND unratify roots. If an authorized caller (MIDNIGHT contract or maker) can set `newIsRootRatified = false` for an active root, all pending orders under that root would immediately become invalid. This could be used to DoS the order book if the authorized caller is compromised or acts maliciously. The `NotRatified` error suggests there's a check that prevents certain operations on unratified roots, but the ability to unratify active roots is itself a risk.

Impact
A compromised or malicious authorized caller could unratify all active roots, effectively canceling all pending orders in the protocol. This is a centralization risk that could be exploited for market manipulation (e.g., canceling all sell orders to drive up prices) or as a DoS attack.
Scenario
1. Attacker compromises the authorized caller (MIDNIGHT contract or privileged role)
2. Attacker calls `setIsRootRatified(maker, activeRoot, false)` for all active makers
3. All pending orders are immediately invalidated
4. Protocol order book is emptied
5. Attacker can then place their own orders at favorable prices
Affected code
function setIsRootRatified(address maker, bytes32 root, bool newIsRootRatified) external;

/// ERRORS ///
error NotRatified(); // suggests unratified roots are checked
Proposed fix
Consider making root ratification one-directional (once ratified, cannot be unratified) or adding a time-lock for unratification:
function setIsRootRatified(address maker, bytes32 root, bool newIsRootRatified) external {
    // Only allow ratification, not unratification (use cancelRoot for that)
    if (!newIsRootRatified) revert CannotUnratify();

    // ... access control checks
    isRootRatified[maker][root] = true;
    emit SetIsRootRatified(msg.sender, maker, root, true);
}
Alternatively, if unratification is needed, add a time-lock mechanism to give makers time to react.
...
IERC20Permit.sol
Floating Pragma Allows Compilation with Vulnerable Compiler Versions
Multiple interface files use overly permissive pragma statements (`pragma solidity >=0.5.0`). This includes `IERC20Permit.sol`, `IPermit2.sol`, `IMidnightBundles.sol`, and `IEcrecoverAuthorizer.sol`. A floating pragma allows these contracts to be compiled with any Solidity version from 0.5.0 onwards, including versions with known bugs or security vulnerabilities. For example, versions prior to 0.8.0 lack built-in overflow/underflow protection, and various compiler versions have had bugs related to ABI encoding, optimizer issues, and more. While these are interfaces, they define critical data structures (like `Authorization`, `Signature`, `Take`, `TokenPermit`) and constants (like `AUTHORIZATION_TYPEHASH`, `EIP712_DOMAIN_TYPEHASH`) that are used in security-critical operations.

Impact
Contracts may be accidentally deployed or compiled using outdated or problematic compiler versions, potentially introducing bugs or security vulnerabilities. The security-critical constants like `AUTHORIZATION_TYPEHASH` and `EIP712_DOMAIN_TYPEHASH` could behave differently across compiler versions.
Scenario
A developer compiling with Solidity 0.5.x would lack overflow protection and other safety features present in 0.8.x. The `Authorization` struct and signature verification logic could behave unexpectedly under older compiler semantics.
Affected code
pragma solidity >=0.5.0; // in IERC20Permit.sol, IPermit2.sol, IMidnightBundles.sol, IEcrecoverAuthorizer.sol
Proposed fix
Lock all pragma statements to a specific version or narrow range consistent with the rest of the codebase. Since `ConstantsLib.sol`, `EventsLib.sol`, and the ratifier interfaces use `pragma solidity ^0.8.0`, the periphery interfaces should be updated to match:
// Change from:
...
IEcrecoverRatifier.sol
Missing Cross-Chain Replay Protection in EcrecoverRatifier Domain Separator
The `IEcrecoverRatifier` defines `EIP712_DOMAIN_TYPEHASH` as `keccak256("EIP712Domain(uint256 chainId,address verifyingContract)")`. This domain type only includes `chainId` and `verifyingContract`, omitting the `name` and `version` fields that are standard in EIP-712 domain separators. While `chainId` prevents cross-chain replay, the absence of `name` and `version` means that if the same contract is deployed at the same address on a fork (e.g., during a chain split or testnet), signatures could be replayed. More critically, if the `EcrecoverRatifier` contract is upgraded or redeployed at the same address (e.g., via CREATE2 with the same salt after self-destruct), old signatures could be replayed against the new contract.

Impact
Signatures created for one deployment of the EcrecoverRatifier could potentially be replayed against another deployment at the same address (e.g., after a contract upgrade via CREATE2 + selfdestruct pattern). This could allow execution of previously-signed but now-invalid offer roots.
Scenario
1. EcrecoverRatifier is deployed at address `0xABC` on chain 1
2. Maker signs a root for this contract
3. Contract is upgraded (selfdestruct + CREATE2 redeploy at same address)
4. Old signature is still valid for the new contract since domain separator only uses chainId + verifyingContract
5. Attacker replays the old signature to ratify offers that the maker no longer intends to honor
Affected code
/// @dev keccak256("EIP712Domain(uint256 chainId,address verifyingContract)").
bytes32 constant EIP712_DOMAIN_TYPEHASH = 0x47e79534a245952e8b16893a336b85a3d9ea9fa8c573f3d803afb92a79469218;
Proposed fix
Include `name` and `version` in the EIP-712 domain separator to provide additional binding:
...
IMidnightBundles.sol
Referral Fee Recipient Can Be Set to Zero Address Causing Fee Loss
The bundle functions in `IMidnightBundles` accept `referralFeePct` and `referralFeeRecipient` parameters. If `referralFeeRecipient` is set to `address(0)` while `referralFeePct > 0`, the referral fee tokens would be sent to the zero address (burned). There is no validation in the interface that `referralFeeRecipient != address(0)` when `referralFeePct > 0`. This could result in unintentional token burning if a caller mistakenly passes `address(0)` as the recipient.

Impact
If `referralFeeRecipient` is `address(0)` and `referralFeePct > 0`, referral fees are permanently burned. While this is a user error rather than a security vulnerability, it represents a fund loss risk for callers who accidentally pass `address(0)`.
Scenario
1. User calls `buyWithUnitsTargetAndWithdrawCollateral` with `referralFeePct = 100` (1%) and `referralFeeRecipient = address(0)` by mistake
2. Bundle executes successfully
3. 1% of the transaction value is sent to `address(0)` and permanently lost
Affected code
function buyWithUnitsTargetAndWithdrawCollateral(
uint256 targetUnits,
uint256 maxBuyerAssets,
...
CollateralWithdrawal[] memory collateralWithdrawals,
address collateralReceiver,
uint256 referralFeePct, // fee percentage
address referralFeeRecipient // could be address(0)
) external;
Proposed fix
Add validation in the implementation to check that if `referralFeePct > 0`, then `referralFeeRecipient != address(0)`:
if (referralFeePct > 0) {
    require(referralFeeRecipient != address(0), "Invalid referral recipient");
}
Also consider adding a maximum cap on `referralFeePct` to prevent excessive fee extraction.
4

ConstantsLib.sol
maxSettlementFee Array Index Out of Bounds Risk
The `maxSettlementFee` function in `ConstantsLib.sol` uses an inline array literal with 7 elements (indices 0-6) and accesses it with an unchecked `index` parameter. If `index >= 7` is passed, Solidity will revert with a panic (array out of bounds), but this is a runtime error rather than a graceful revert with a meaningful error message. More importantly, the function is `pure` and free functions in Solidity don't have access modifiers, meaning any contract can call it with any index value. If this function is called from a context where the index is derived from user input without prior validation, it could cause unexpected reverts.

Impact
If `index >= 7` is passed, the transaction reverts with a panic error (0x32 - array out of bounds). While this prevents incorrect fee calculations, it could cause DoS in functions that call `maxSettlementFee` with user-controlled indices. The lack of a meaningful error message makes debugging difficult.
Scenario
// Calling with index = 7 causes panic revert
uint256 fee = maxSettlementFee(7); // reverts with Panic(0x32)
Affected code
...
Proposed fix
Add explicit bounds checking with a meaningful error:
error InvalidSettlementFeeIndex(uint256 index);

function maxSettlementFee(uint256 index) pure returns (uint256) {
    if (index >= 7) revert InvalidSettlementFeeIndex(index);
    return [MAX_SETTLEMENT_FEE_0_DAYS, MAX_SETTLEMENT_FEE_1_DAY, MAX_SETTLEMENT_FEE_7_DAYS,
            MAX_SETTLEMENT_FEE_30_DAYS, MAX_SETTLEMENT_FEE_90_DAYS, MAX_SETTLEMENT_FEE_180_DAYS,
            MAX_SETTLEMENT_FEE_360_DAYS][index];
}
5

ConstantsLib.sol
maxLif Division by Zero Risk When lltv Equals WAD and cursor Equals WAD
The `maxLif` function computes `WAD / (WAD - mulDivDown(cursor, WAD - lltv, WAD))`. If `lltv = WAD (1e18)` and `cursor > 0`, then `WAD - lltv = 0`, making `mulDivDown(cursor, 0, WAD) = 0`, so the denominator becomes `WAD - 0 = WAD`, which is fine. However, if `lltv < WAD` and `cursor = WAD`, then `mulDivDown(WAD, WAD - lltv, WAD) = WAD - lltv`, making the denominator `WAD - (WAD - lltv) = lltv`. This is also fine. But consider: if `cursor = WAD` and `lltv = 0` (though 0 is excluded from allowed LLTVs), the denominator would be 0. More critically, if `mulDivDown` returns a value equal to `WAD` due to rounding, the denominator becomes 0, causing a division by zero panic.

Impact
If the denominator `WAD - mulDivDown(cursor, WAD - lltv, WAD)` equals zero, the function panics with a division by zero error. This would cause any liquidation or LIF calculation to revert, potentially blocking liquidations and allowing undercollateralized positions to persist.
Scenario
// With LLTV_8 = WAD = 1e18 and cursor = LIQUIDATION_CURSOR_HIGH = 0.5e18:
// mulDivDown(0.5e18, WAD - WAD, WAD) = mulDivDown(0.5e18, 0, WAD) = 0
// denominator = WAD - 0 = WAD (safe)

// Edge case: if cursor could be set to WAD (1e18) and lltv = LLTV_0 = 0.385e18:
// mulDivDown(WAD, WAD - 0.385e18, WAD) = WAD - 0.385e18 = 0.615e18
// denominator = WAD - 0.615e18 = 0.385e18 (safe)

// The risk is if cursor > LIQUIDATION_CURSOR_HIGH is somehow possible
// or if mulDivDown rounds up to WAD
Affected code
...
Proposed fix
Add a check to ensure the denominator is non-zero:
function maxLif(uint256 lltv, uint256 cursor) pure returns (uint256) {
    uint256 cursorAdjusted = UtilsLib.mulDivDown(cursor, WAD - lltv, WAD);
    require(cursorAdjusted < WAD, "maxLif: denominator would be zero");
    return UtilsLib.mulDivDown(WAD, WAD, WAD - cursorAdjusted);
}
Also ensure that `cursor` is always bounded by `LIQUIDATION_CURSOR_HIGH` before calling `maxLif`.
...
ConstantsLib.sol
Gas Optimization: Inline Array in maxSettlementFee Creates Memory Allocation on Every Call
The `maxSettlementFee` function creates an inline array literal `[MAX_SETTLEMENT_FEE_0_DAYS, ..., MAX_SETTLEMENT_FEE_360_DAYS]` on every call. In Solidity, inline array literals are allocated in memory on each function call, which costs gas proportional to the array size. Since all values are compile-time constants, this memory allocation is unnecessary. A more gas-efficient approach would use a series of `if-else` statements or a `mapping` of constants.

Impact
Each call to `maxSettlementFee` allocates a 7-element array in memory, costing approximately 7 * 32 = 224 bytes of memory allocation plus MSTORE operations. This is wasteful since all values are constants. In high-frequency operations (e.g., fee calculations during every take), this adds unnecessary gas overhead.
Scenario
// Current: allocates 7-element array in memory on every call
function maxSettlementFee(uint256 index) pure returns (uint256) {
    return [MAX_SETTLEMENT_FEE_0_DAYS, ..., MAX_SETTLEMENT_FEE_360_DAYS][index];
    // Gas: ~200+ gas for memory allocation + MSTORE operations
}

// Optimized: uses if-else chain, no memory allocation
function maxSettlementFee(uint256 index) pure returns (uint256) {
    if (index == 0) return MAX_SETTLEMENT_FEE_0_DAYS;
    if (index == 1) return MAX_SETTLEMENT_FEE_1_DAY;
    // ... etc
    // Gas: ~50-100 gas for comparisons
}
Affected code
/// @dev Returns the max settlement fee for the given index.
function maxSettlementFee(uint256 index) pure returns (uint256) {
...
Proposed fix
Replace the inline array with an if-else chain or switch-like pattern:
function maxSettlementFee(uint256 index) pure returns (uint256) {
    if (index == 0) return MAX_SETTLEMENT_FEE_0_DAYS;
    if (index == 1) return MAX_SETTLEMENT_FEE_1_DAY;
    if (index == 2) return MAX_SETTLEMENT_FEE_7_DAYS;
    if (index == 3) return MAX_SETTLEMENT_FEE_30_DAYS;
    if (index == 4) return MAX_SETTLEMENT_FEE_90_DAYS;
    if (index == 5) return MAX_SETTLEMENT_FEE_180_DAYS;
    if (index == 6) return MAX_SETTLEMENT_FEE_360_DAYS;
    revert InvalidSettlementFeeIndex(index);
}
This eliminates memory allocation and adds bounds checking.
...
ConstantsLib.sol
MAX_CONTINUOUS_FEE Precision Loss Due to Integer Division
The `MAX_CONTINUOUS_FEE` constant is computed as `uint32(uint256(0.01e18) / uint256(365 days))`. This division truncates the result: `0.01e18 / 31536000 = 317097919.8...` which truncates to `317097919`. The truncation means the actual maximum continuous fee rate is slightly lower than the intended 1% per year. While the difference is small (~0.000000025% per year), this represents a precision loss in a security-critical constant. Additionally, the cast to `uint32` could silently truncate if the value exceeds `2^32 - 1 = 4294967295`, though in this case `317097919 < 4294967295` so no overflow occurs.

Impact
The maximum continuous fee is slightly lower than intended (317097919 vs 317097919.8... per second in WAD units). This is a minor precision issue that slightly under-caps the maximum fee, which is actually conservative (safer for users). However, it demonstrates a pattern of precision loss that could be more significant in other calculations.
Scenario
// Intended: 1% per year = 0.01e18 / 31536000 = 317097919.837...
// Actual: 317097919 (truncated)
// Difference: 0.837 units per second in WAD
// Over 1 year: 0.837 * 31536000 = ~26,395,392 WAD units ≈ 0.0000000264% error
uint32 constant MAX_CONTINUOUS_FEE = uint32(uint256(0.01e18) / uint256(365 days));
// = uint32(317097919) -- truncated from 317097919.837...
Affected code
uint32 constant MAX_CONTINUOUS_FEE = uint32(uint256(0.01e18) / uint256(365 days));
Proposed fix
Document the precision loss explicitly, or use a higher precision intermediate:
// Option 1: Document the truncation
/// @dev 1% per year in WAD per second, truncated from 317097919.837...
uint32 constant MAX_CONTINUOUS_FEE = uint32(uint256(0.01e18) / uint256(365 days));

// Option 2: Use a slightly higher value to round up (more conservative cap)
uint32 constant MAX_CONTINUOUS_FEE = uint32((uint256(0.01e18) + uint256(365 days) - 1) / uint256(365 days));
// = 317097920 (rounds up)
2

IEcrecoverAuthorizer.sol
Inconsistent Pragma Versions Between Core and Periphery Contracts
The codebase uses two different pragma version strategies: `pragma solidity ^0.8.0` for core contracts (`ConstantsLib.sol`, `EventsLib.sol`, `ISetterRatifier.sol`, `IEcrecoverRatifier.sol`) and `pragma solidity >=0.5.0` for periphery interfaces (`IERC20Permit.sol`, `IPermit2.sol`, `IMidnightBundles.sol`, `IEcrecoverAuthorizer.sol`). This inconsistency creates a risk where the periphery contracts could be compiled with a different (potentially incompatible or vulnerable) compiler version than the core contracts. The ABI encoding behavior, overflow semantics, and other language features differ significantly between Solidity 0.5.x and 0.8.x.

Impact
Inconsistent compiler versions could lead to ABI encoding mismatches, different overflow behavior, and other subtle bugs when periphery contracts interact with core contracts. This is particularly concerning for the `IEcrecoverAuthorizer` which defines security-critical constants and structs.
Scenario
A developer compiling `IEcrecoverAuthorizer.sol` with Solidity 0.5.x would get different behavior for:
- Struct encoding (no `abi.encode` in 0.5.x)
- Integer overflow (no built-in protection in 0.5.x)
- `bytes32` constant computation
Affected code
// Core contracts:
pragma solidity ^0.8.0; // ISetterRatifier.sol, IEcrecoverRatifier.sol, ConstantsLib.sol, EventsLib.sol

// Periphery interfaces:
pragma solidity >=0.5.0; // IERC20Permit.sol, IPermit2.sol, IMidnightBundles.sol, IEcrecoverAuthorizer.sol
Proposed fix
Standardize all pragma statements to `pragma solidity ^0.8.0` across the entire codebase:
// All files should use:
pragma solidity ^0.8.0;
This ensures consistent behavior, overflow protection, and compiler optimizations across all contracts.
3

EventsLib.sol
Take Event Missing Indexed Fields for Critical Parameters
The `Take` event in `EventsLib.sol` has many parameters but only 3 can be indexed (Solidity limitation). The current indexing choices are `id_`, `taker`, and `maker`. However, `caller` (the address initiating the take, which could be a bundle contract) and `group` (which appears to be a critical grouping parameter for consumed tracking) are not indexed. This makes it difficult to efficiently query events by caller or group off-chain, which could impact monitoring, analytics, and incident response capabilities.

Impact
Off-chain systems cannot efficiently filter `Take` events by `caller` or `group` without scanning all events. This impacts monitoring tools, analytics dashboards, and incident response systems that need to track activity by specific callers or groups. The `group` field is particularly important as it relates to the `SetConsumed` event and consumed tracking.
Scenario
An off-chain monitoring system trying to find all takes by a specific caller (e.g., a bundle contract) would need to download all `Take` events and filter client-side, which is inefficient and expensive for high-volume protocols.
Affected code
event Take(
address caller, // NOT indexed
bytes32 indexed id_,
uint256 units,
address indexed taker,
address indexed maker,
bool offerIsBuy,
bytes32 group, // NOT indexed - critical for consumed tracking
uint256 buyerAssets,
...
Proposed fix
Consider restructuring the event to index the most query-critical fields. Since Solidity limits indexed fields to 3, prioritize based on query patterns:
// Option 1: Index group instead of maker (if group queries are more common)
event Take(
    address caller,
    bytes32 indexed id_,
    uint256 units,
    ...
    address maker,
    bool offerIsBuy,
    bytes32 indexed group,  // index group for consumed tracking queries
    // ... rest of params
);

// Option 2: Split into multiple events for different query patterns
// Or use a separate indexed mapping for group -> takes
Alternatively, emit a separate lightweight event with indexed fields for the most common query patterns.
4

ConstantsLib.sol
LIQUIDATION_LOCK_SLOT Uses keccak256 of String - Potential Storage Collision
The `LIQUIDATION_LOCK_SLOT` constant is defined as `uint256(keccak256("morpho.midnight.liquidationLocked"))`. This pattern (using a keccak256 hash of a string as a storage slot) is commonly used in proxy contracts (EIP-1967) to avoid storage collisions. However, if this slot is used directly in assembly (`sstore`/`sload`) rather than through a Solidity mapping, there's a risk that future contract upgrades or additional storage variables could accidentally collide with this slot. The probability is astronomically low for a random collision, but if the contract uses a proxy pattern, the proxy's implementation slot could theoretically conflict.

Impact
If another storage variable or proxy implementation slot collides with `LIQUIDATION_LOCK_SLOT`, the liquidation lock mechanism could be corrupted, potentially allowing reentrancy during liquidations or preventing legitimate liquidations. The probability of accidental collision is negligible, but intentional collision by a malicious proxy admin is theoretically possible.
Scenario
The slot value is `uint256(keccak256("morpho.midnight.liquidationLocked"))`. An attacker with proxy admin access could potentially deploy an implementation contract where a storage variable at this exact slot position is used for a different purpose, corrupting the liquidation lock.
Affected code
uint256 constant LIQUIDATION_LOCK_SLOT = uint256(keccak256("morpho.midnight.liquidationLocked"));
Proposed fix
This pattern is generally safe and follows EIP-1967 best practices. However, document the slot value explicitly and ensure it's used consistently:
/// @dev Storage slot for the liquidation lock, following EIP-1967 pattern.
/// Value: keccak256("morpho.midnight.liquidationLocked") = 0x...
/// Used in assembly: sstore(LIQUIDATION_LOCK_SLOT, 1) to lock, sstore(LIQUIDATION_LOCK_SLOT, 0) to unlock
uint256 constant LIQUIDATION_LOCK_SLOT = uint256(keccak256("morpho.midnight.liquidationLocked"));
Ensure the implementation contract is not upgradeable, or if it is, use a comprehensive storage layout audit before any upgrade.
```

---

# Integrated PoC Queue From This Scan

```text
test/poc/PoC_EcrecoverAuthorizerZeroSigner.t.sol
test/poc/PoC_BundlerAuthorizationScope.t.sol
test/poc/PoC_BundlerReentrancy.t.sol
test/poc/PoC_RootDelegateGriefing.t.sol
test/poc/PoC_MaxSettlementFeeBounds.t.sol
test/poc/PoC_MaxLifDenominator.t.sol
test/poc/PoC_PermitFrontRunTolerance.t.sol
```

## Foundry commands

```bash
forge test --match-path 'test/poc/PoC_EcrecoverAuthorizerZeroSigner.t.sol' -vvvv
forge test --match-path 'test/poc/PoC_BundlerAuthorizationScope.t.sol' -vvvv
forge test --match-path 'test/poc/PoC_BundlerReentrancy.t.sol' -vvvv
forge test --match-path 'test/poc/PoC_RootDelegateGriefing.t.sol' -vvvv
forge test --match-path 'test/poc/PoC_MaxSettlementFeeBounds.t.sol' -vvvv
forge test --match-path 'test/poc/PoC_MaxLifDenominator.t.sol' -vvvv
forge test --match-path 'test/poc/PoC_PermitFrontRunTolerance.t.sol' -vvvv
```

---

# Final Cumulative Audit Priority

After combining all scans and the Midnight knowledge graph, prioritize:

```text
1. Core Midnight.take callback and post-callback accounting.
2. Core Midnight.liquidate badDebt/lossFactor/withdrawable accounting.
3. Periphery temporary-balance / cross-function reentrancy.
4. Periphery exact-equality rounding and pre-try/catch route DoS.
5. Ratifier delegate cancellation / unratification griefing.
6. EcrecoverAuthorizer zero-signer and nonce/deadline verification if implementation exists.
7. Permit2 / authorized relayer operation-context misuse.
8. ConstantsLib bounds and mathematical denominator checks.
9. Oracle/gate liveness only if not fully documented or forced on victims.
10. Events, NatSpec, pragma, gas findings last.
```

---

# Cantina-Ready Validation Rule

Do **not** submit AI scan text directly. For each candidate, produce:

```text
1. exact implementation line range,
2. violated invariant,
3. attacker prerequisites,
4. Foundry PoC,
5. impact quantification,
6. likelihood explanation,
7. dedup status against comments/audits/Certora/tests.
```
...
## Appendix: `defi_protocol_periphery_bundler_aggregator_hashlock_ai_audit_combined_report.md`
